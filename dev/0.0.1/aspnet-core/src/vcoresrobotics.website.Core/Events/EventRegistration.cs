...
namespace vcoresrobotics.website.Events
{
    ...
    public class EventRegistration : CreationAuditedEntity, IMustHaveTenant
    {
        ...
        public async Task CancelAsync(IRepository<EventRegistration> repository)
        {
            ...
            if (Event.IsInPast()) { throw new UserFriendlyException("Can not cancel event which is in the past!"); }

            if (Event.IsAllowedCancellationTimeEnded()) { throw new UserFriendlyException("It's too late to cancel your registration!"); }
